...
class Lunar{
     ...
     * @return string
     */
    public function SolarTimeToLunarTime(int $hour){
        $hourInfo=['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥'];
        if($hour>1){
            $key=ceil($hour/2);
        }else{
            $key=0;
        }
        return $hourInfo[$key].'时';

    }
    /**新历转农历
     * @param int $year 新历-年
     * @param int $month 新历-月
     * @param int $date 新历-日
     * @return array
     */
     ...
     * @return array
     */
   public function convertSolarMonthToLunar(int $year,int $month,int $date=0):array {
        $yearData=$this->lunarInfo[$year-$this->MIN_YEAR];
        if($year==$this->MIN_YEAR&&$month<=2){
            return array(1891,'正月','初一','辛卯',1,1,'兔');
        }
        ...
        for ($i=1;$i<$dd;$i++){
            ...
            $array['date']=$year.'-'.$month.'-'.$i;
            $lunar_ary[$i]=$array;
        }
        if($date){
            return $lunar_ary[$date];
        }
        return $lunar_ary;
    }
    /**农历转新历
     ...
     * @param int $date 农历日期
     * @param bool $is_leap 是否闰月
     * @return array
     */
   public function convertLunarToSolar($year,$month,$date,bool $is_leap=false){
        $leap_month=$this->getLeapMonth($year);
        if($month>$leap_month ||$is_leap==true){
            $month=$month+1;//大于闰月的日期处理：例如如果当年闰五月，那么第二个五月就传六月，相当于农历有13个月，只是有的时候第13个月的天数为0
        }
        $yearData=$this->lunarInfo[$year-$this->MIN_YEAR];
   ...
   public function isLeapYear(int $year):bool {
     ...
     * @return string
     */
  public  function getLunarYearName(int $year){
     ...
     * @return string 农历年
     */
  public  function getYearZodiac(int $year){
     ...
     * @return int|mixed
     */
   public function getLunarMonthDays(int $year,int $month,bool $is_leap=false){
        $monthData=$this->getLunarMonths($year);
       $leap_month=$this->getLeapMonth($year);
       if($month>$leap_month ||$is_leap==true){
           $month=$month+1;//大于闰月的日期处理
       }
     ...
     * @return mixed
     */
    public function getLunarYearDays(int $year){
     ...
     * @return array
     */
   private function getLunarYearMonths($year){
        $monthData=$this->getLunarMonths($year);
        $res=array();
        $yearData=$this->lunarInfo[$year-$this->MIN_YEAR];
     ...
     * @return int
     */
   public function getLeapMonth(int $year){
        $yearData=$this->lunarInfo[$year-$this->MIN_YEAR];
        return $yearData[0];
    }
     ...
     * @return int|mixed
     */
    public  function getDaysBetweenLunar(int $year,int $month,int $date,bool $is_leap=false){
     ...
     * @param int $cmonth
     * @param int $cdate
     * @param int $dmonth 农历正月对应的新历月份
     * @param int $ddate 农历初一对应的新历天数
     * @return float
     */
    ...
    private function getLunarByBetween(int $year,int $between){
        ...
        if($between==0){
            ...
            $lunarArray=['year'=>$year,'lunar_month_str'=>$m,'lunar_date_str'=>$this->getCapitalNum($e,false)];
        }
        $lunarArray['chronology']=$this->getLunarYearName($year);// 天干地支
        $lunarArray['lunar_month']=($leapMonth!=0&&$t==$leapMonth+1)?$t-1:$t;// 新历月份
        $lunarArray['lunar_date']=$e;// 新历日期
        $lunarArray['zodiac']=$this->getYearZodiac($year);// 12生肖
        $lunarArray['leap_month']=$leapMonth;// 闰几月
        return $lunarArray;
    }
    /**农历日期
     * @param $num  int 数字
     ...
     * @return string
     */
    private function getCapitalNum($num,$isMonth){
